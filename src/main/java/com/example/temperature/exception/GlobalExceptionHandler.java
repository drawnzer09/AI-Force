...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<Object> handleMaxUploadSizeExceeded(MaxUploadSizeExceededException ex) {
        return buildResponse(
                ...
                List.of(new ErrorDetail(null, "Request payload exceeds the configured maximum size"))
        );
    }

    @ExceptionHandler(DataAccessException.class)
    public ResponseEntity<Object> handleDataAccess(DataAccessException ex) {
        LOGGER.error("Database access failure", ex);
        return buildResponse(
                HttpStatus.SERVICE_UNAVAILABLE,
                "SERVICE_UNAVAILABLE",
                "The service is temporarily unable to store or retrieve temperature data",
                List.of()
        );
    }
    ...
    private ErrorDetail toErrorDetail(ConstraintViolation<?> violation) {
        return new ErrorDetail(violation.getPropertyPath().toString(), violation.getMessage());
    }

    private String resolveMessage(DefaultMessageSourceResolvable error) {
